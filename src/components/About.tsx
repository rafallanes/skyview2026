            ...
            {queObtienes.map((it, i) => (
              <Reveal key={it.n} delay={i * 0.08}>
                <div className="border border-zinc-200 p-10 -ml-px -mt-px h-full">
                  <div className="text-6xl font-light text-zinc-200 mb-4 leading-none">{it.n}</div>
                ...
                {diagnosisItems.map((item, i) => (
                  <Reveal key={i} delay={i * 0.08}>
                    <div className="group relative border-t border-gray-200 py-10 md:py-14 pr-8 overflow-hidden">
                      <span className="absolute top-3 right-3 text-[5rem] md:text-[6rem] font-bold text-foreground/[0.12] select-none pointer-events-none leading-none">
                        {item.ghost}
                      </span>
